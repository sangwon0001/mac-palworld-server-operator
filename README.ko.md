...
앱 UI 274개 문자열이 세 언어로 제공됩니다. **설정(⚙️) → 언어** 에서 바꾸며,
재시작 없이 즉시 반영됩니다. 기본값은 시스템 언어를 따르고, 셋 중 없으면 영어입니다.

구현 메모:

- **한국어 원문을 그대로 키로 씁니다.** `t("서버 시작")` 처럼요. 키를 따로 짓지
  않아 코드가 읽히는 그대로이고, 번역이 빠진 항목은 한국어가 그대로 나옵니다.
- Xcode 없이 `swiftc` 로 빌드하므로 **String Catalog(`.xcstrings`)는 쓸 수 없습니다.**
  고전적인 `app/Resources/<lang>.lproj/Localizable.strings` 를 번들에 복사합니다.
- `NSLocalizedString` 은 시스템 언어를 따라 앱 안에서 못 바꾸므로, 해당 `.lproj`
  번들을 직접 열어 조회합니다. 그래서 재시작이 필요 없습니다.
- `SettingsCatalog` 의 119개 설정 라벨은 **데이터로 두고 화면에 낼 때 번역**합니다.
  static 사전 안에서 번역하면 첫 접근 시점에 굳어 언어 전환이 반영되지 않습니다.

> 일본어는 기계 번역이 아니라 직접 작성했지만, 원어민 검수는 거치지 않았습니다.
> 어색한 표현이 있으면 `app/Resources/ja.lproj/Localizable.strings` 를 고치고
> `cd app && ./build.sh --install` 로 다시 빌드하면 됩니다.
...
```bash
./backup_save.sh --list                       # 백업 목록 (이름 포함)
./backup_save.sh --name "보스전 직전"          # 이름을 붙여 백업
./backup_save.sh --rename 20260805_1130 "중요"  # 이름 바꾸기
./backup_save.sh --rename 20260805_1130 ""     # 이름 지우기
```

### 백업에 이름 붙이기

이름을 주면 `palworld_backup_<시각>_<이름>.tar.gz` 로 저장됩니다.

> **이름이 붙은 백업은 자동 정리에서 제외됩니다.** 이름을 지었다는 건 남겨 두고
> 싶다는 뜻이므로, 보관 기간(14일)이 지나도 지우지 않습니다. 이름을 지우면
> 다시 자동 정리 대상이 됩니다.

`--rename` 의 대상은 파일명 전체 또는 타임스탬프 일부(`20260805_1130`)로 지정합니다.
여러 개가 걸리면 실행하지 않고 알려 줍니다.

이름은 파일명에 안전하도록 다듬어집니다 — 공백은 밑줄로, `/ \ : * ? " < > |` 는
밑줄로 바뀌고 40자로 제한됩니다. 한글은 그대로 쓸 수 있습니다.

> 길이 제한은 **문자 단위**입니다. `cut -c` 는 바이트 단위라 한글이 중간에서 잘려
> 깨진 파일명이 만들어지므로 python3 로 처리합니다.

이름은 타임스탬프 **뒤에** 붙습니다. 그래야 기존 glob(`palworld_backup_*.tar.gz`)과
날짜 파싱(앞 15자 고정폭)이 그대로 통해서, 복원·상태 조회·자동 정리가 모두
영향을 받지 않습니다.

앱에서는 백업 칸의 입력란에 이름을 적고 [지금 백업] 을 누르면 되고,
각 백업 줄의 [이름] 버튼으로 나중에 바꿀 수도 있습니다.
...
잘못 복원해도 되돌아갈 수 있습니다. 최근 3개는 항상 남기고, 그보다 오래된 것은
보관 기간이 지나면 일반 백업과 함께 정리됩니다.

확인 프롬프트를 건너뛰려면 `--yes` 를 붙입니다(앱이 복원할 때 쓰는 방식입니다).

```bash
./restore_save.sh --yes palworld_backup_20260804_050000.tar.gz
```
...
시간별 백업이 정각이 아니라 30분인 이유가 있습니다. `auto_restart.sh` 는 서버를
끄기 전에 자체적으로 백업을 뜨므로, 정각에 두면 매일 05:00 · 06:00 작업과 같은
분에 겹칩니다. 그래도 겹치는 경우(예: cron 작업 도중 앱에서 버튼을 누름)에는
