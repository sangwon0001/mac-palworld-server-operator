...
if mode == "set":
    ...
    # 정밀 치환: 해당 키의 값만 바꾸고 나머지 원문은 손대지 않습니다.
    new_body = body
    for k, old, new in changes:
        pattern = re.compile(r'(?<![A-Za-z0-9_])' + re.escape(k) + r'=' + re.escape(old) + r'(?=,|\)|$)')
...
case "$MODE" in
  ...
  *) die "알 수 없는 옵션: $MODE (--json | --get | --set | --diff)" ;;
esac
